...
#define COLOR_RAMP(colors,factor,finalColor) {\
  int index=0;\
  for(int i=0;i<2;i++){ColorStop cc=colors[i];bool inB=cc.position<=factor;index=int(mix(float(index),float(i),float(inB)));}\
...
export default function Aurora({
  colorStops = ['#3A1C71', '#00d4ff', '#22c55e'],
  amplitude = 1.0,
  blend = 0.5,
  speed = 0.5,
}: AuroraProps) {
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uTime.value = t * 0.001 * (p.speed ?? 0.5) * 0.1;
      program.uniforms.uAmplitude.value = p.amplitude ?? 1.0;
      program.uniforms.uBlend.value = p.blend ?? 0.5;
      program.uniforms.uColorStops.value = (p.colorStops ?? colorStops).map(h => {
        const c = new Color(h); return [c.r, c.g, c.b];
      });
  ...
  return <div ref={ctnRef} className="absolute inset-0 w-full h-full" />;
}
